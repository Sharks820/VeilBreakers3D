...
|----------|-------|----------|
| CRITICAL | 5 | Performance/Production readiness/Design/Bug |
| HIGH | 11 | Performance/Memory/Lifecycle/Data |
...
### C5. `_isEmbarking` Not Reset on OperationCanceledException (BUG)
**File:** `Assets/Scripts/UI/CharacterSelect/CharacterSelectManager.cs:691-703`
**Severity:** CRITICAL

```csharp
private async void TriggerEmbark()
{
    if (_isEmbarking) return;  // Guard
    _isEmbarking = true;
    try {
        await ExecuteEmbarkAsync(hero);
    }
    catch (OperationCanceledException) {
        Debug.Log("Embark cancelled");
        // BUG: _isEmbarking NOT reset! Permanently locks embark button.
    }
    catch (Exception ex) {
        _isEmbarking = false;  // Only this path resets it
    }
}
```

After any timeout or object destruction cancellation, `_isEmbarking` stays `true` forever, **permanently locking out the embark button**. The player cannot embark again without restarting the scene.

**Fix:** Add `_isEmbarking = false;` to the OperationCanceledException catch block, or use a `finally` block:
```csharp
finally { _isEmbarking = false; }
```
...
### H10. Shared Mutable Buffer Return in PathSystem (Data Corruption Risk)
**File:** `Assets/Scripts/Systems/PathSystem.cs:32-36`
**Severity:** HIGH
...
public static Dictionary<Stat, float> GetPathBonuses(Path path, float pathLevel)
{
    ...
    _bonusBuffer.Clear();  // Clears previous caller's data!
    ...
    return _bonusBuffer;   // Returns shared reference
}
```

Returns a shared static dictionary. Any caller that stores the result across multiple calls gets silently corrupted data when the next call overwrites the buffer. The comment says "do not cache the reference across calls" but this is fragile.

**Fix:** Return a new Dictionary, or use a struct/ReadOnlyDictionary pattern. The [ThreadStatic] helps with threading but not with sequential call corruption.

---

### H11. Enum Casting Without Validation in SkillData
**File:** `Assets/Scripts/Data/SkillData.cs:88-101`

```csharp
public SkillType GetSkillType() => (SkillType)skill_type;     // Raw int cast
public DamageType GetDamageType() => (DamageType)damage_type;  // Raw int cast
public TargetType GetTargetType() => (TargetType)target_type;  // Raw int cast
```

JSON-deserialized ints cast directly to enums without `Enum.IsDefined()` validation. Invalid data produces undefined enum values that silently fall through switch statements (e.g., BattleManager.ExecuteAbility's switch on SkillType). The skill consumes MP but does nothing.

**Fix:** Add validation or use a default case that logs a warning.
...
1. **C5** - Fix `_isEmbarking` not reset on OperationCanceledException (embark button lockout)
2. **C4** - Unify dual StatusEffect systems (Combatant._statusEffects vs StatusEffectManager)
3. **C2** - Change `StatusEffectManager._debugLogging` default to `false`
4. **C1** - Gate SaveManager/GameDatabase/MigrationRunner Debug.Logs behind conditional compilation
...
*Total findings: 5 Critical, 11 High, 8 Medium, 6 Low, 7 Optimization Opportunities (37 total)*
